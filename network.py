...
class AE_Net(nn.Module):
    def __init__(self, img_ch=3, output_ch=1):
        ...
        # self.Conv5 = conv_block(ch_in=512, ch_out=1024)
        self.Conv2 = conv_block(ch_in=64, ch_out=128)
        self.Conv3 = conv_block(ch_in=128, ch_out=128)
        self.Conv4 = conv_block(ch_in=128, ch_out=32)
        self.Conv5 = conv_block(ch_in=32, ch_out=8)
        self.Conv6 = conv_block(ch_in=8, ch_out=1)

        self.Up6 = up_conv(ch_in=1, ch_out=8)
        self.Up_conv6 = conv_block(ch_in=8, ch_out=8)

        self.Up5 = up_conv(ch_in=8, ch_out=32)
        self.Up_conv5 = conv_block(ch_in=32, ch_out=32)

        self.Up4 = up_conv(ch_in=32, ch_out=128)
...
class FreeU_Net(nn.Module):
    ...
    def __init__(self, img_ch=3, output_ch=1, b1=1.4, b2=1.6, s1=0.9, s2=0.2):
        super(FreeU_Net, self).__init__()
    ...
    def forward(self, x):
        ...
        # Apply Free U-Net modifications in decoder layers
        # --------------- Free U-Net Code -----------------------
        if d4.shape[1] == 1280:
            hidden_mean = d4.mean(1).unsqueeze(1)
            B = hidden_mean.shape[0]
            hidden_max, _ = torch.max(hidden_mean.view(B, -1), dim=-1, keepdim=True) 
            hidden_min, _ = torch.min(hidden_mean.view(B, -1), dim=-1, keepdim=True)
            hidden_mean = (hidden_mean - hidden_min.unsqueeze(2).unsqueeze(3)) / (hidden_max - hidden_min).unsqueeze(2).unsqueeze(3)
            d4[:,:640] = d4[:,:640] * ((self.b1 - 1 ) * hidden_mean + 1)
            d4 = Fourier_filter(d4, threshold=1, scale=self.s1)
        if d4.shape[1] == 640:
            hidden_mean = d4.mean(1).unsqueeze(1)
            B = hidden_mean.shape[0]
            hidden_max, _ = torch.max(hidden_mean.view(B, -1), dim=-1, keepdim=True) 
            hidden_min, _ = torch.min(hidden_mean.view(B, -1), dim=-1, keepdim=True)
            hidden_mean = (hidden_mean - hidden_min.unsqueeze(2).unsqueeze(3)) / (hidden_max - hidden_min).unsqueeze(2).unsqueeze(3)
            d4[:,:320] = d4[:,:320] * ((self.b2 - 1 ) * hidden_mean + 1)
            d4 = Fourier_filter(d4, threshold=1, scale=self.s2)
        # --------------------------------------------------------
        ...
        # --------------- Free U-Net Code -----------------------
        if d3.shape[1] == 1280:
            hidden_mean = d3.mean(1).unsqueeze(1)
            B = hidden_mean.shape[0]
            hidden_max, _ = torch.max(hidden_mean.view(B, -1), dim=-1, keepdim=True) 
            hidden_min, _ = torch.min(hidden_mean.view(B, -1), dim=-1, keepdim=True)
            hidden_mean = (hidden_mean - hidden_min.unsqueeze(2).unsqueeze(3)) / (hidden_max - hidden_min).unsqueeze(2).unsqueeze(3)
            d3[:,:640] = d3[:,:640] * ((self.b1 - 1 ) * hidden_mean + 1)
            d3 = Fourier_filter(d3, threshold=1, scale=self.s1)
        if d3.shape[1] == 640:
            hidden_mean = d3.mean(1).unsqueeze(1)
            B = hidden_mean.shape[0]
            hidden_max, _ = torch.max(hidden_mean.view(B, -1), dim=-1, keepdim=True) 
            hidden_min, _ = torch.min(hidden_mean.view(B, -1), dim=-1, keepdim=True)
            hidden_mean = (hidden_mean - hidden_min.unsqueeze(2).unsqueeze(3)) / (hidden_max - hidden_min).unsqueeze(2).unsqueeze(3)
            d3[:,:320] = d3[:,:320] * ((self.b2 - 1 ) * hidden_mean + 1)
            d3 = Fourier_filter(d3, threshold=1, scale=self.s2)
        # --------------------------------------------------------
